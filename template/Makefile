.PHONY: start-worker execute installdeps start-examples execute-insurance-claims execute-cargo-release execute-email-triage execute-code-modernization
...
start-examples:
	cd src && uv run python -m examples.worker
...
execute-insurance-claims:
	uv run python src/workflows/start.py --workflow insurance-claims-triage $(if $(input),--input '$(input)',--input '{"claim_id":"CLM-2024-001","claimant_name":"Maria Gonzalez","description":"My car was T-boned at an intersection.","photos":["examples/insurance_claims/sample_data/photos/claim_high_totaled_front.jpg","examples/insurance_claims/sample_data/photos/claim_high_totaled_side.jpg"]}')
...
execute-cargo-release:
	uv run python src/workflows/start.py --workflow cargo-release-compliance $(if $(input),--input '$(input)',--input '{"document_uri":"examples/cargo_release/sample_data/shipping_doc_normal.png","shipment_id":"BL-2024-HAM-009371"}')
...
execute-code-modernization:
	uv run python src/workflows/start.py --workflow code-modernization $(if $(input),--input '$(input)',--input '{"repo_path":"examples/code_modernization/sample_data/legacy_repo","target":"Python 2.7 → 3.12"}')
